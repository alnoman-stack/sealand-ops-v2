...
import { Eye, Edit, Trash2, FileText, Printer, X, Save, Search, Plus, Download, Calendar as CalendarIcon, Loader2 } from 'lucide-react';
...
export default function OrdersView() {
  ...
  const [isDownloading, setIsDownloading] = useState(false); // ✅ ডাউনলোডিং স্টেট
  ...
  // ✅ সুপার-ফাস্ট PDF ডাউনলোড ফাংশন (html2canvas)
  ...
  return (
    <div className="space-y-6 relative p-4 bg-black min-h-screen text-white font-sans">
      
      {/* Header & Date Filter */}
      <div className="flex justify-between items-center bg-[#0a0a0a] p-6 rounded-[2rem] border border-gray-900 shadow-2xl">
        <div className="flex flex-col gap-1">
          <h2 className="text-xl font-black uppercase italic tracking-tighter text-white">Orders History</h2>
        ...
        {orders.length > 0 ? orders.map(order => (
          <div key={order.id} className="bg-[#0a0a0a] border border-gray-900 p-6 rounded-[2rem] flex justify-between items-center group shadow-xl transition-all hover:border-blue-500/50">
            ...
            <div className="flex items-center gap-3">
               <button onClick={() => handleViewOrder(order)} className="p-3 bg-white/5 rounded-xl text-green-500 hover:bg-green-500/10 transition-all"><Eye size={18} /></button>
               <button onClick={() => handleEditOrder(order)} className="p-3 bg-white/5 rounded-xl text-blue-500 hover:bg-blue-500/10 transition-all"><Edit size={18} /></button>
               <button onClick={() => handleDirectPrint(order)} className="p-3 bg-white/5 rounded-xl text-gray-400 hover:bg-white/10 transition-all"><Printer size={18} /></button>
               <button onClick={() => deleteOrder(order.id)} className="p-3 bg-white/5 rounded-xl text-red-500 hover:bg-red-500/10 transition-all"><Trash2 size={18} /></button>
            </div>
          </div>
        )) : (
          ...
          <div className="bg-white w-full max-w-4xl h-[90vh] rounded-3xl overflow-hidden shadow-2xl flex flex-col">
            ...
            <div className="p-6 bg-gray-50 flex justify-end gap-4 border-t">
              <button 
                ...
                className="bg-blue-600 hover:bg-blue-700 disabled:bg-blue-400 text-white px-8 py-3 rounded-xl flex items-center gap-2 font-black uppercase text-[11px] tracking-widest transition-all shadow-lg"
              >
                {isDownloading ? <><Loader2 size={16} className="animate-spin" /> Processing...</> : <><Download size={16} /> Download PDF</>}
              </button>
              <button onClick={() => handlePrint()} className="bg-gray-800 hover:bg-black text-white px-8 py-3 rounded-xl flex items-center gap-2 font-black uppercase text-[11px] tracking-widest transition-all"><Printer size={16} /> Print</button>
            </div>
          </div>
        </div>
      )}
               ...
               <div className="w-1/3 border-r border-gray-900 p-6 overflow-y-auto bg-black/20">
                  ...
                  <div className="space-y-2">
                    {filteredProducts.map((p, index) => (
                       <div key={p.id || index} className="p-4 bg-white/5 border border-gray-900 rounded-2xl flex justify-between items-center hover:border-blue-500/50 transition-all">
                          <div>
                          ...
                          <button onClick={() => addNewItem(p)} className="bg-blue-600 hover:bg-blue-500 p-2 rounded-xl text-white transition-colors"><Plus size={16}/></button>
                       </div>
                    ))}
                  </div>
               </div>
               ...
               <div className="flex-1 p-8 flex flex-col bg-white/5">
                            ...
                            {orderItems.map((item, i) => (
                              <tr key={i} className="border-b border-gray-900/50 hover:bg-white/5 transition-colors">
                                 ...
                                 <td className="p-5 text-center">
                                    <input type="number" step="any" className="w-20 bg-white/5 border border-gray-800 rounded-lg p-2 text-center text-white font-black outline-none focus:border-blue-500" value={item.qty} onChange={(e) => updateItemQty(i, e.target.value)}/>
                                 </td>
                  ...
                  <div className="mt-8 flex justify-between items-center bg-black border border-gray-900 p-8 rounded-[2rem] shadow-2xl">
                    ...
                    <button onClick={saveUpdatedOrder} disabled={isSaving} className="bg-blue-600 hover:bg-blue-500 text-white px-10 py-4 rounded-2xl flex items-center gap-2 font-black uppercase text-[11px] tracking-widest shadow-xl transition-all active:scale-95 disabled:opacity-50">
                       ...
                       {isSaving ? "Saving..." : "Update & Save Invoice"}
                    </button>
                  </div>
               </div>
            </div>
      ...
      {/* Hidden container for PDF/Print Generation */}
